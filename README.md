...
- 范围统计分析：首次打开插件视图时自动刷新当天新增/修改文件，之后按需复用短时缓存，也可手动切换到最近 7 天 / 最近 30 天；若是 Git 仓库，会补充对应时间范围内提交的删文件与增删行统计
...
- 扩展改为按视图或命令按需激活，避免启动时立刻扫描
- 范围统计仅在插件视图可见或用户主动刷新时更新，并且会短时复用最近一次结果，避免频繁重复分析
